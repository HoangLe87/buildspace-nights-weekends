...
contract MasterChefV2 is SpookyAuth, SelfPermit, Multicall, ReentrancyGuard {
        ...
        returns (uint256 pending)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 booReward = totalAllocPoint == 0
                ? 0
                : ((multiplier * booPerSecond() * pool.allocPoint) /
                    totalAllocPoint);
            accBooPerShare =
                accBooPerShare +
                ((booReward * ACC_BOO_PRECISION) / lpSupply);
        }
        pending =
            ((user.amount * accBooPerShare) / ACC_BOO_PRECISION) -
            user.rewardDebt;
    }
    ...
    function booPerSecond() public view returns (uint256 amount) {
        uint256 totalAlloc = MASTER_CHEF.totalAllocPoint();
        if (totalAlloc > 0)
            amount =
                (MASTER_CHEF.booPerSecond() *
        ...
        returns (PoolInfo memory pool)
    {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                ...
                pool.accBooPerShare = (pool.accBooPerShare +
                    ((booReward * ACC_BOO_PRECISION) / lpSupply)).toUint128();
            }
    ...
    function _deposit(
        ...
        // Effects
        uint256 _pendingBoo = ((user.amount * pool.accBooPerShare) /
            ACC_BOO_PRECISION) - user.rewardDebt;

        user.amount += amount;
        user.rewardDebt =
            (user.amount * pool.accBooPerShare) /
            ACC_BOO_PRECISION;
    ...
    function setV1HarvestQueryTime(uint256 newTime, bool inDays)
        external
        onlyAuth
    {
