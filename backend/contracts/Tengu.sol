...
contract TenguToken is BEP20 {
    ...
    event SensitiveOperatorTransferred(
        address indexed previousOperator,
        address indexed newOperator
    );
    ...
    event SwapGTenguToTengu(
        address indexed sender,
        address indexed recipient,
        uint256 gTenguAmount,
        uint256 tenguAmount
    );
    ...
    modifier onlySensitiveOperator() {
        require(
            _sensitiveOperator == msg.sender,
            "operator: caller is not the sensitive operator"
        );
        _;
    }
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "TENGU::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    function _transfer(
        ...
        if (
            ...
            transferTaxRate == 0 ||
            excludedFromTransferTax[sender] ||
            excludedFromTransferTax[recipient]
        ) {
            ...
            require(
                ...
                "TENGU::transfer: Tax value invalid"
            );
            ...
            uint256 liquidityAmount = taxAmount.sub(burnAmount).sub(
                toGTenguAmount
            );
            require(
                taxAmount == burnAmount + toGTenguAmount + liquidityAmount,
                "TENGU::transfer: Burn value invalid"
            );
            ...
            super._transfer(
                sender,
                address(this),
                liquidityAmount.add(toGTenguAmount)
            );
    ...
    function swapAndLiquify() external lockTheSwap transferTaxFree {
        require(
            address(tenguSwapRouter) != address(0),
            "TENGU::swapAndLiquify: Router not defined"
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        require(
            locker != address(0),
            "TENGU::addLiquidity: locker address must be set"
        );
        ...
        onlyOperator
    {
        require(
            ...
            "TENGU::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function updateBurnRate(uint256 _burnRate) public onlyOperator {
        require(
            _burnRate.add(gTenguRate) <= 100,
            "TENGU::updateBurnRate: GTengu + burn rates must not exceed the maximum rate."
        );
    ...
    function updateGTenguRate(uint256 _gTenguRate) public onlyOperator {
        require(
            burnRate.add(_gTenguRate) <= 100,
            "TENGU::updateGTenguRate: GTengu + burn rates must not exceed the maximum rate."
        );
     ...
     * Can only be called by the current operator.
     */
    function updateMaxTransferAmountRate(uint256 _maxTransferAmountRate)
        public
        onlyOperator
    {
        require(
            _maxTransferAmountRate <= 50,
            "TENGU::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
        require(
            _maxTransferAmountRate >= MIN_TRANSFER_AMOUNT_RATE,
            "TENGU::updateMaxTransferAmountRate: Min transfer amount rate must be above the minimum rate."
        );
     ...
     */
    function updateTenguSwapRouter(address _router)
        public
        onlySensitiveOperator
    {
        tenguSwapRouter = IUniswapV2Router02(_router);
        tenguSwapPair = IUniswapV2Factory(tenguSwapRouter.factory()).getPair(
            address(this),
            tenguSwapRouter.WETH()
        );
        require(
            tenguSwapPair != address(0),
            "TENGU::updateTenguSwapRouter: Invalid pair address."
        );
        emit TenguSwapRouterUpdated(
            msg.sender,
            address(tenguSwapRouter),
            tenguSwapPair
        );
    }
    ...
    function updateLocker(address _locker) public onlySensitiveOperator {
        require(
            _locker != address(0),
            "TENGU::updateTenguLocker: new operator is the zero address"
        );
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(
            ...
            "TENGU::transferOperator: new operator is the zero address"
        );
     ...
     * Can only be called by the current sensitiveOperator.
     */
    function transferSensitiveOperator(address newOperator)
        public
        onlySensitiveOperator
    {
        require(
            newOperator != address(0),
            "TENGU::transferSensitiveOperator: new operator is the zero address"
        );
    ...
    function _swapGTenguToTengu(
        ...
        require(
            gTengu.balanceOf(sender) >= amount,
            "TENGU::swapGTenguToTengu: not enough GTENGU"
        );
    ...
    function setSwapGTenguToTenguFee(uint256 fee) external onlyOperator {
        require(
            fee <= SWAP_GTENGU_TO_TENGU_MAX_FEE,
            "TENGU::setSwapGTenguToTenguFee: fee too high"
        );
     ...
     */
    function getSwapGTenguToTenguAmount(uint256 amount)
        public
        view
        returns (uint256 tenguAmount)
    {
     ...
     */
    function setGTenguContractAddress(IGreatTengu gTengu_)
        external
        onlyOperator
    {
        require(
            address(gTengu) == address(0),
            "TENGU::setGTenguContractAddress: already initialized"
        );
     ...
     * Can only be called by the current operator.
     */
    function setExcludedFromGTenguTax(address _account, bool _excluded)
        public
        onlyOperator
    {
    ...
    function buyBackAndBurn() external onlyOperator {
        ...
        // make the swap
        tenguSwapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: ethAmount
        }(
