...
contract PacocaFarm is Ownable, ReentrancyGuard {
    ...
    uint256 public constant ownerPACOCAReward = 200; // 15% Dev + 5% MKT
    ...
    function pendingPACOCA(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && sharesTotal != 0) {
            ...
            uint256 PACOCAReward = multiplier
                .mul(PACOCAPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
        ...
        return
            user.shares.mul(accPACOCAPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    */
    ...
    function addPool(
        ...
        require(
            !availableAssets[_want],
            "Can't add another pool of same asset"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
