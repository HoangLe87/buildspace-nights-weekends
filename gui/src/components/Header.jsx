...
import { checkIdExistsAndCreate } from '@/utils/firestore'
...
export const Header = () => {
  const connectWallet = async () => {
    try {
      ...
      checkIdExistsAndCreate(accounts[0])
    } catch (error) {
      toast('Opps:', error)
    }
  }
  ...
  const changeAccount = async () => {
    try {
      ...
      const connectedAccount = account[0].caveats[0].value[0]
      setCurrentAccount(connectedAccount)
      checkIdExistsAndCreate(connectedAccount)
    } catch (error) {
      toast(`Opps: ${error}`)
    }
  }
      ...
      <div className="w-full bg-gray-700/80 py-5 pr-5">
        ...
        <nav className="z-1 relative flex justify-between">
          ...
          <div className="flex items-center gap-x-5 md:gap-x-8">
            ...
            {currentAccount && (
              <Button color="gradient" onClick={() => changeAccount()}>
