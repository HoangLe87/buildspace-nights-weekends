...
export function SwapBox({ pairs, setPairs }) {
  const tokens = pairs.map((pair) => pair.token2).concat('ANNA')
  const [tokenInput1, setTokenInput1] = useState([...tokens])
  const [tokenInput2, setTokenInput2] = useState(tokens)
  console.log(tokens)
  console.log(tokenInput1)
  console.log(tokenInput2)
  const toggleToken1 = (e) => {
    setTokenInput1(tokens.filter((token) => token != e.target.value))
  }
  const toggleToken2 = (e) => {
    setTokenInput2(tokens.filter((token) => token != e.target.value))
  }
        ...
        <div className="relative transform overflow-hidden rounded-lg bg-gray-500 px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
          <div>
            <div className="mt-3 text-center sm:mt-5">
              ...
              <div className="mb-2 border-2 border-solid border-green-500 bg-green-500 text-sm font-bold text-white">
                <div>Buy</div>
              ...
              <div className="border-2 border-solid border-red-500 bg-red-500 text-sm font-bold text-white">
                <div>Sell</div>
                <div className="mt-2 flex">
                  <select
                    id="token2Symbol"
